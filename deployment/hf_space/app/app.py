...
def setup_page_config() -> None:
    ...
    st.set_page_config(
        page_title="MediScan — AI Skin Lesion Classification (Research Prototype)",
        page_icon="🔬",
        layout="wide",
...
def render_sidebar(device: torch.device) -> Dict[str, Any]:
    ...
    with st.sidebar:
        st.title("🔬 MediScan")
        st.caption("Automated Skin Lesion Classification Prototype (Research & Education)")
        st.markdown("---")
...
def main() -> None:
    ...
    st.markdown(
        ...
        <div class="main-header">
            <h2>🔬 MediScan — Skin Lesion Classification Prototype</h2>
            <p style="color: #64748b; font-size: 1.05rem;">
                Research & Educational Platform for Automated Dermatoscopic Pattern Analysis (HAM10000)
            </p>
        </div>
        """,
        unsafe_allow_html=True,
        ...
        st.markdown(
            ...
            <div style="background-color: {risk_info['badge_bg']}; border-left: 5px solid {risk_info['badge_color']}; padding: 1.25rem; border-radius: 8px; margin-bottom: 1rem;">
                <span style="color: {risk_info['badge_color']}; font-weight: 700; font-size: 0.85rem; text-transform: uppercase;">
                    Predicted Class (HAM10000 Category)
                </span>
    ...
    try:
        ...
        st.warning(f"⚠️ Grad-CAM visualization encountered an issue: {e}. Model predictions above remain valid.")
    ...
    if gradcam_success and gradcam_res is not None:
        ...
        with g_col3:
            st.image(
                gradcam_res["overlay"],
                caption="[3] Grad-CAM Explanation Overlay",
                width="stretch",
            )
    ...
    # 6. Structured Exportable Summary
    st.subheader("📑 Structured Prediction Export (JSON)")
    summary_data = {
        "source": image_source_label,
        "model": "EfficientNet-B0 (Day 9 Locked Checkpoint)",
        "intended_use": "Educational and research prototype (non-diagnostic)",
        "prediction": {
            "predicted_code": pred_code,
            "predicted_description": prediction_res["predicted_description"],
            "category_type": risk_info["level"],
    ...
    st.download_button(
        label="📥 Download Structured Prediction Report (JSON)",
        data=summary_json,
        file_name=f"mediscan_report_{pred_code}_{int(confidence_pct)}pct.json",
        mime="application/json",
        help="Export machine-readable prediction findings and probability distribution.",
    )

    # 7. Clinical & Educational Disclaimer
    st.markdown(
        """
        <div class="disclaimer-box">
            <strong>⚠️ RESEARCH & EDUCATIONAL PROTOTYPE DISCLAIMER:</strong><br>
            This application is an educational and research prototype for automated dermatoscopic image classification. 
            It is <strong>not a medical diagnostic tool</strong> and must <strong>not be used to make clinical decisions</strong>. 
            Predictions may be incorrect and performance may vary across populations, devices, acquisition settings, and clinical environments.<br><br>
            <strong>Known Algorithmic & Dataset Limitations:</strong><br>
            • <strong>HAM10000 Dataset:</strong> Trained and evaluated exclusively on the HAM10000 research benchmark (Vienna & Queensland cohorts; predominantly fair-skinned European populations; Fitzpatrick phototypes not available).<br>
            • <strong>No Clinical Clearance:</strong> The system has not undergone prospective clinical trials, peer medical review, or regulatory clearance (FDA/CE).<br>
            • <strong>Confidence ≠ Certainty:</strong> High softmax probability does not equal clinical diagnostic certainty. Overconfident misclassifications occur (as audited in Day 14, 13 melanoma cases were misclassified as nevus with &gt;80% confidence).<br>
            • <strong>Class Imbalance:</strong> Extreme 58:1 imbalance (67% benign nevi) creates strong baseline prior probabilities favoring benign classification.<br>
            • <strong>Domain Shift:</strong> Performance on non-dermatoscope images, smartphone cameras, or uncalibrated illumination is unverified and expected to degrade significantly.
        </div>
        """,
        unsafe_allow_html=True,
    )
